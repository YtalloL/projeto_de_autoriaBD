...
async function cadastrarGenero(event) {
  ...
  const GeneroValor = document.getElementById("Genero_Nome_Genero_formulario").value;
  const novoGenero = { Nome_Genero: GeneroValor };
...
async function renderizarListaGeneros(componentePrincipal) {
  try {
    ...
    const tbgeneroBD = await response.json(); 

    const generos = tbgeneroBD.map((row) => {
      return {
        id: row.IdGenero,
    ...
    inserirEventosExcluirGenero();
    inserirEventosAtualizarGenero();
  } catch (error) {
 ...
 */
function inserirEventosExcluirGenero() {
 ...
 */
function inserirEventosAtualizarGenero() {
...
async function buscarGenero(id) {
  try {
    ...
    const tbgeneroBD = await response.json();
    if (tbgeneroBD.length <= 0) return;

    const genero = tbgeneroBD.map(row => ({
      id: row.IdGenero,
      Nome_Genero: row.Nome_Genero,
...
async function atualizarGenero(event) {
  ...
  const GeneroValor = document.getElementById("Genero_Nome_Genero_formulario").value;
  const genero = {id: idValor, Nome_Genero: GeneroValor};
